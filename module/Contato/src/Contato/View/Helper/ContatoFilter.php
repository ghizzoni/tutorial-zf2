...
class ContatoFilter extends AbstractHelper
{
    ...
    {
        $partes_nomes = explode(" ", $this->nomeCompleto());
        $result = null;
        
        if (count($partes_nomes) <= 2) {
            $result = join($partes_nomes, " ");
        } else {
            $result = "{$partes_nomes[0]} {$partes_nomes[1]} ...";
        }
    ...
    {
        $result = (new \DateTime($this->contato->data_atualizacao))->format('d/n/Y - H:i');
